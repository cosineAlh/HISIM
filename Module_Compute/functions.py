...
class imc_analysis():
    def __init__(self, xbar_size, volt, freq, freq_adc, quant_bits, RELU):
        super(imc_analysis, self).__init__()
    ...
    def forward(self, data, layer_idx, network_params):
        ...
        L_adc_xbar = self.L_single_adc*(adc_col_res_factor**2/(1/256**2))*self.ADC_factor*factor_lat
        ...
        Qavg = self.Qaccum+math.ceil(num_stages/2)                 

        if n_c_x == 1:
            num_adders = 0
        else:
            num_adders = n_c_x/2*num_stages*self.xbar_y/self.Qw*n_c_y
        ...
        L_bus_t = self.L_bus*bus_len*(util_col/self.xbar_y+self.Qaccum/self.Qact*util_row/self.xbar_x)*input_cycle/(self.C_i*self.C_w*self.st**2)*10 # Latency of the bus 
        E_bus_t = self.E_bus*bus_len*(n_c_x+n_c_y*self.Qaccum/self.Qact)*input_cycle/(self.C_i*self.C_w*self.st**2)/self.xbar_x                      # Energy of the bus
        if self.relu == True:
            L_activation = 4e-9
        else:
