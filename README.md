# HISIM_V1.1
HISIM introduces a suite of analytical models at the system level to speed up performance prediction for AI models, covering logic-on-logic architectures across 2D and 3D integration.
...
├── Module_Thermal          -- Folder containing thermal simulation code for 2D and 3D architectures
├── Results
...
--chip_architect    set_chip_architecture     Chip Architecture options are 2D chip (M2D), 3D chip (M3D).
...
--N_stack,          set_N_stack               Number of 3D stacks.
```

### Running the 2D/3D simulations
#### 2D Simulation
...
```
freq_core (GHz),freq_noc (GHz),Xbar_size,N_tile,N_pe,N_tile(real),N_tier(real),N_stack(real),W2d,W3d,Computing_latency (ns),Computing_energy (pJ),compute_area (um2),chip area (mm2),chip_Architecture,2d NoC latency (ns),3d NoC latency (ns),network_latency (ns),2d NoC energy (pJ),3d NoC energy (pJ),network_energy (pJ),rcc (compute latency/communciation latency),Throughput(TFLOPS/s),2D_3D_NoC_power (W),2d_3d_router_area (mm2),placement_method,percent_router,peak_temperature (C),thermal simulation time (s),networking simulation time (s),computing simulation time (s),total simulation time (s)
```
...
network_latency(ns):   Total Network Latency consisting of 2D NoC, 3D TSV interface latencies
2d NoC energy(pJ):     Total Energy of the 2D NoC
3d NoC energy(pJ):     Total Energy of the 3D TSV
network_energy(pJ):    Total Network Energy consisting of 2D NoC, 3D TSV interface energies
...
Test Case 1    PPA           ViT             1024-9-100-2-2-3D                  NA - Single run
Test Case 2    PPA           densenet121     1024-36-64-2-2-3D                  NA - Single run
