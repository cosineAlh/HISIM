...
class util_map():
    ...
    def forward(self, layer_num_tile, layer_idx):
        ...
        if self.placement_method == 2:
            # Placement method 2: tile-to-tile 3D connection. 
            ...
            if self.tiles_each_tier[self.stack_index][self.tier_index]+layer_num_tile > self.N_tile:
                total_tiles_required_layer = self.N_tile-self.tiles_each_tier[self.stack_index][self.tier_index]+layer_num_tile
                if self.tier_index == self.N_tier-1:
                    ...
                    self.tier_index += 1
            self.tiles_each_tier[self.stack_index][self.tier_index] += layer_num_tile
...
def create_tile(ax, x, y, z, dx, dy, dz, color, label):
    ...
    poly = Poly3DCollection(verts, facecolors=color, alpha=0.3, edgecolor='lightblue')
    ax.add_collection3d(poly)
    ax.text(x + dx/2, y + dy/2, z + dz, label, color='black', ha='center', va='center', fontsize=10)
